<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_responsi = mysqli_query($con,"SELECT * FROM `ikgm_nilai_responsi` WHERE `nim`='$nim_mhsw_ikgm' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_responsi = mysqli_fetch_array($daftar_responsi))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI RESPONSI KASUS KECIL KEPANITERAAN (STASE) IKGM");
			...
			$tabel2{5} = array ('item'=>"Nama Pasien", 'isi'=>": "."$data_responsi[nama_pasien]");
			$tabel2{6} = array ('item'=>"No CM Pasien", 'isi'=>": "."$data_responsi[cm_pasien]");
			$tabel2{7} = array ('item'=>"Diagnosis", 'isi'=>": "."$data_responsi[diagnosis]");
