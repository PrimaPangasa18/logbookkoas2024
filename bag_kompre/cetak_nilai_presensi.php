<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_presensi = mysqli_query($con,"SELECT * FROM `kompre_nilai_presensi` WHERE `nim`='$nim_mhsw_kompre' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_presensi = mysqli_fetch_array($daftar_presensi))
		{
		  ...
		  $tabel1{2} = array ('item'=>"KEPANITERAAN KOMPREHENSIP");
		  ...
		  $tabel2{5} = array ('item'=>"Lokasi Puskesmas/Rumah Sakit", 'isi'=>": "."$data_presensi[lokasi]");
		...
		$tabel7{2} = array ('item'=>"KEPANITERAAN KOMPREHENSIP");
		...
		//Rekap Rumah Sakit
		$kolom8b = array ('ITEM'=>'');
		$tabel8b{1} = array ('ITEM'=>'<b>Rekap Rata-Rata Nilai Presensi / Kehadiran di Rumah Sakit</b>');
		...
		$tabel10a{1} = array ('NO'=>'1','UNSUR'=>$nilai_masuk,'SKOR'=>$rata_nilai_masuk_rumkit);
		$tabel10a{2} = array ('NO'=>'2','UNSUR'=>$nilai_absen,'SKOR'=>$rata_nilai_absen_rumkit);
		$tabel10a{3} = array ('NO'=>'3','UNSUR'=>$nilai_ijin,'SKOR'=>$rata_nilai_ijin_rumkit);
		...
		$tabel11a{1} = array ('TOTAL'=>$nilai_total,'NILAI'=>"<b>$total_presensi_rumkit</b>");
		...
		$rata_total_presensi=($total_presensi_puskesmas+$total_presensi_rumkit)/2;
		...
		//Persetujuan Kordik
		$nip_kordik = mysqli_fetch_array(mysqli_query($con,"SELECT `username` FROM `admin` WHERE `stase`='K122'"));
