...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_kasus = mysqli_query($con, "SELECT * FROM `ipd_nilai_kasus` WHERE `nim`='$nim_mhsw_ipd' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_kasus = mysqli_fetch_array($daftar_kasus)) {
			...
			$tabel1{
			2} = array('item' => "KEPANITERAAN (STASE) ILMU PENYAKIT DALAM");
			$tabel1{
			...
			$tabel4b{
			1} = array('NO' => '1', 'ASPEK' => 'Slide, sikap, suara', 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_1]);
			$pdf->ezTable(
				$tabel4b,
				$kolom4b,
				"",
			...
			$tabel4d{
			1} = array('NO' => '2', 'ASPEK' => 'Kesesuaian format dan kedalaman pembahasan untuk dokter umum', 'BOBOT' => '20%', 'NILAI' => $data_kasus[aspek_2]);
			$tabel4d{
			2} = array('NO' => '3', 'ASPEK' => 'Kepustakaan', 'BOBOT' => '5%', 'NILAI' => $data_kasus[aspek_3]);
			$pdf->ezTable(
				$tabel4d,
				$kolom4d,
				"",
			...
			//Nilai Aspek Diskusi: Aspek 4-10
			$kolom4e = array('ASPEK' => '');
			$tabel4e{
			...
			$tabel4f{
			1} = array('NO' => '4', 'ASPEK' => "Anamnesis (<i>sacred seven, fundamental four</i>)", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_4]);
			$tabel4f{
			2} = array('NO' => '5', 'ASPEK' => "Pemeriksaan Fisik (<i>status lokalis dan generalis</i>)", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_5]);
			$tabel4f{
			3} = array('NO' => '6', 'ASPEK' => "Pemeriksaan Penunjang (<i>jenis dan interpretasi hasil</i>)", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_6]);
			$tabel4f{
			4} = array('NO' => '7', 'ASPEK' => "Diagnosis Banding / Diagnosis Kerja", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_7]);
			$tabel4f{
			5} = array('NO' => '8', 'ASPEK' => "Tatalaksana", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_8]);
			$tabel4f{
			6} = array('NO' => '9', 'ASPEK' => "Komplikasi, Pencegahan, Prognosis, Tindak lanjut di FKTP", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_9]);
			$tabel4f{
			7} = array('NO' => '10', 'ASPEK' => "Diskusi lain (patogenesis, farmakologi dsb)", 'BOBOT' => '5%', 'NILAI' => $data_kasus[aspek_10]);
			$pdf->ezTable(
				$tabel4f,
				$kolom4f,
				"",
