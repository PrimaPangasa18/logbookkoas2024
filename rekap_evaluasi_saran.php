...
<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3))
	{
	  ...
	  if ($_COOKIE['level']==3) {include "menu3.php";}
		...
		echo "<center><h4><font style=\"color:#006400;text-shadow:1px 1px black;\">REKAP EVALUASI AKHIR STASE - UMUM<br>KOMENTAR, KRITIK DAN SARAN</font></h4>";
		...
		echo "<thead>
					...
					<th style=\"width:50%\">Komentar, Kritik, dan Saran</th>
					</thead>";
		$id = 1;
		$kelas = "ganjil";
		while ($nim_mhsw = mysqli_fetch_array($mhsw))
		{
			...
			echo "<tr class=$kelas>";
				...
				if ($jml_data_evaluasi>0) echo "<td>$komentar[komentar]</td>";
