	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3))
	{
	  ...
	  if ($_COOKIE['level']==3) {include "menu3.php";}
		...
		echo "<thead>
					...
					<th style=\"width:25%\">Lokasi Stase Luar</th>
					<th style=\"width:10%\">Lama Stase</th>
					<th style=\"width:30%\">Hal yang Disukai</th>
					<th style=\"width:30%\">Hal yang Tidak Disukai</th>
					</thead>";
		$id = 1;
		$kelas = "ganjil";
		while ($nim_mhsw = mysqli_fetch_array($mhsw))
		{
			...
			if ($jml_data_evaluasi>0)
			{
				echo "<tr class=$kelas>";
					echo "<td>$id</td>";
					echo "<td>$stase_luar[lokasi_luar]</td>";
					echo "<td align=center>$stase_luar[lama_luar] minggu</td>";
					echo "<td>$stase_luar[like_luar]</td>";
					echo "<td>$stase_luar[unlike_luar]</td>";
				echo "</tr>";
