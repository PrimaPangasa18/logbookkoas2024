...
$data_koas = mysqli_query($con,"SELECT * FROM `jadwal_10agt2020`");
$no = 0;
while ($data=mysqli_fetch_array($data_koas))
{
  ...
  for ($i = 1; $i < 10; $i++)
  {
    ...
    if ($stase!="")
    {
      ...
      if ($ada_jadwal>0)
      {
        $update_jadwal=mysqli_query($con,"UPDATE `$stase_id` SET `rotasi`='9',`tgl_mulai`='$tgl_mulai',`tgl_selesai`='$tgl_selesai',`status`='0',`evaluasi`='0' WHERE `nim`='$nim_mhsw'");
      ...
      else
      {
        $insert_jadwal=mysqli_query($con,"INSERT INTO `$stase_id`
          ( `nim`, `rotasi`, `tgl_mulai`, `tgl_selesai`,
            `status`, `evaluasi`)
          VALUES
          ( '$nim_mhsw','9','$tgl_mulai','$tgl_selesai',
            '0','0')");
