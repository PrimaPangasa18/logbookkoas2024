<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_jurnal = mysqli_query($con,"SELECT * FROM `mata_nilai_jurnal` WHERE `nim`='$nim_mhsw_mata' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_jurnal = mysqli_fetch_array($daftar_jurnal))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI PRESENTASI JOURNAL READING KEPANITERAAN (STASE) ILMU KESEHATAN MATA");
			...
			$tabel2{5} = array ('item'=>"Judul Artikel", 'isi'=>": "."<i>$data_jurnal[judul_presentasi]</i>");
			...
			$tabel4{2} = array ('NO'=>'','ASPEK'=>'1.1. Penampilan','NMAKS'=>'10%','NILAI'=>$data_jurnal[aspek_1]);
			$tabel4{3} = array ('NO'=>'','ASPEK'=>'1.2. Penyampaian','NMAKS'=>'20%','NILAI'=>$data_jurnal[aspek_2]);
			$tabel4{4} = array ('NO'=>'','ASPEK'=>'1.3. Makalah','NMAKS'=>'20%','NILAI'=>$data_jurnal[aspek_3]);
			$tabel4{5} = array ('NO'=>'2','ASPEK'=>'Penguasaan Materi','NMAKS'=>'30%','NILAI'=>$data_jurnal[aspek_4]);
			$tabel4{6} = array ('NO'=>'3','ASPEK'=>'Pengetahuan Teori / Penunjang','NMAKS'=>'20%','NILAI'=>$data_jurnal[aspek_5]);
			...
			$tabel5{1} = array ('TOTAL'=>'Nilai Total (Jumlah Bobot x Nilai):','NILAI'=>$data_jurnal[nilai_total]);
			...
			while ($i<6)
			{
				...
				if ($data_jurnal[$penyanggah_i]!="-")
				{
					...
					$tabel5a{$row_i} = array ('ITEM'=>"<i>$i. $mhsw[nama] (NIM: $mhsw[nim]) [Nilai: $data_jurnal[$nilai_penyanggah_i]]</i>");
				}
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Presentasi Journal Reading          <i>[hal $halaman dari $jumlah_halaman hal]</i>");
