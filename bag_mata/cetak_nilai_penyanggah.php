<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_penyanggah = mysqli_query($con,"SELECT * FROM `mata_nilai_penyanggah` WHERE `nim`='$nim_mhsw_mata' AND `status_approval`='1' ORDER BY `jenis_presentasi` DESC,`tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_penyanggah);
		$halaman = 1;
		while ($data_penyanggah = mysqli_fetch_array($daftar_penyanggah))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI PENYANGGAH PRESENTASI KEPANITERAAN (STASE) ILMU KESEHATAN MATA");
			...
			$tabel2{5} = array ('item'=>"Jenis Presentasi", 'isi'=>": "."$data_penyanggah[jenis_presentasi]");
			$tabel2{6} = array ('item'=>"Judul Presentasi", 'isi'=>": "."$data_penyanggah[judul_presentasi]");
			$data_presenter = mysqli_fetch_array(mysqli_query($con,"SELECT `nim`,`nama` FROM `biodata_mhsw` WHERE `nim`='$data_penyanggah[presenter]'"));
			$tabel2{7} = array ('item'=>"Presenter", 'isi'=>": "."$data_presenter[nama] (NIM: $data_presenter[nim])");
			$tabel2{8} = array ('item'=>"Nilai", 'isi'=>": "."$data_penyanggah[nilai]");
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Penyanggah Presentasi                         <i>[hal $halaman dari $jumlah_halaman hal]</i>");
