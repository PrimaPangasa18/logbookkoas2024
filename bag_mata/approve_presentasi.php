...
<head>
	...
	<title>On-line Approval Presentasi Kasus Logbook Koas Pendidikan Dokter FK-UNDIP</title>
		...
		<div class="main">
			...
			<main class="content px-3 py-4" style="background-image: url('images/undip_watermark_color.png'), url('images/undip_watermark_color.png'); ">
				<div class="container-fluid">
					<div class="mb-3">
						...
						<br>
						<h2 class="fw-bold fs-4 mb-3 text-center" style="color:#0A3967">APPROVAL NILAI PRESENTASI KASUS BESAR<br>KEPANITERAAN (STASE) ILMU KESEHATAN MATA</h2>
						<br>
						<?php
						$id_stase = "M104";
						$id = $_GET['id'];
						...
						echo "<td class=\"td_mid\"><input type=\"text\" class=\"form-select tanggal_penyajian\" name=\"tanggal_penyajian\" style=\"font-size:1em;font-family:Poppins;border:0.5px solid black;border-radius:5px;\" value=\"$data_presentasi[tgl_penyajian]\" /></td>";
						echo "</tr>";
						//Judul
						echo "<tr class=\"table-primary\" style=\"border-width: 1px; border-color: #000;\">";
						echo "<td><strong>Judul Presentasi Kasus</strong></td>";
						echo "<td><textarea name=\"judul_presentasi\" style=\" width: 100%;font-family:Poppins;border:0.5px solid black;border-radius:5px;\" required>$data_presentasi[judul_presentasi]</textarea></td>";
						echo "</tr>";
						//Dosen Penilai
						echo "<tr class=\"table-success\" style=\"border-width: 1px; border-color: #000;\">";
						...
						echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"aspek1\" style=\"width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" id=\"aspek1\" value=\"$data_presentasi[aspek_1]\" onkeyup=\"sum();\" onchange=\"sum();\" required/></td>";
						echo "</tr>";
						//No 1.2
						echo "<tr class=\"table-warning\" style=\"border-width: 1px; border-color: #000;\">";
						echo "<td align=center>&nbsp;</td>";
						...
						echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"aspek2\" style=\"width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" id=\"aspek2\" value=\"$data_presentasi[aspek_2]\" onkeyup=\"sum();\" onchange=\"sum();\" required/></td>";
						echo "</tr>";
						//No 1.3
						echo "<tr class=\"table-warning\" style=\"border-width: 1px; border-color: #000;\">";
						echo "<td align=center>&nbsp;</td>";
						...
						echo "<td align=center><strong>20%</strong></td>";
						echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"aspek3\" style=\"width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" id=\"aspek3\" value=\"$data_presentasi[aspek_3]\" onkeyup=\"sum();\" onchange=\"sum();\" required/></td>";
						echo "</tr>";
						//2. Penguasaan Materi
						echo "<tr class=\"table-warning\" style=\"border-width: 1px; border-color: #000;\">";
						...
						echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"aspek4\" style=\"width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" id=\"aspek4\" value=\"$data_presentasi[aspek_4]\" onkeyup=\"sum();\" onchange=\"sum();\" required/></td>";
						echo "</tr>";
						...
						echo "<td align=center><strong>20%</strong></td>";
						echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"aspek5\" style=\"width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" id=\"aspek5\" value=\"$data_presentasi[aspek_5]\" onkeyup=\"sum();\" onchange=\"sum();\" required/></td>";
						echo "</tr>";

						//Nilai Total
						echo "<tr class=\"table-success\" style=\"border-width: 1px; border-color: #000;\">";
						echo "<td colspan=3 align=right><strong>Nilai Total <span class=\"text-danger\">(Jumlah Bobot x Nilai)</span></strong></td>";
						echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"nilai_total\" style=\"width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" id=\"nilai_total\" value=\"$data_presentasi[nilai_total]\" required disabled/></td>";
						echo "</tr>";
						echo "</table><br>";
						...
						while ($i < 6) {
							...
							if ($data_presentasi[$penyanggah_i] == "-")
								...
								$mhsw_i = mysqli_fetch_array(mysqli_query($con, "SELECT `nim`,`nama` FROM `biodata_mhsw` WHERE `nim`='$data_presentasi[$penyanggah_i]'"));
							...
							echo "</select>";
							echo "</td>";
							if ($data_presentasi[$nilai_penyanggah_i] == "-") echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"$nilai\" style=\"margin-top:17px;width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" placeholder=\"0-100\" /></td>";
							else echo "<td align=center><input type=\"number\" step=\"0.01\" min=\"0\" max=\"100\" name=\"$nilai\" style=\"margin-top:17px;width:55%;border:0.2px solid black;border-radius:3px;font-size:0.85em;text-align:center\" value=\"$data_presentasi[$nilai_penyanggah_i]\" /></td>";
							echo "</tr>";
							$i++;
						}
						echo "</table><br>";
			...
			";
						...
						if ($_POST['approve'] == "APPROVE") {
							$dosen_presentasi = mysqli_fetch_array(mysqli_query($con, "SELECT `dosen` FROM `mata_nilai_presentasi` WHERE `id`='$_POST[id]'"));
							...
							if (($_POST['dosenpass'] != "" and $dosenpass_md5 == $user_dosen['password'])
								...
								$update_presentasi = mysqli_query($con, "UPDATE `mata_nilai_presentasi` SET
					...
					WHERE `id`='$_POST[id]'");
								...
								while ($i < 6) {
									if (!empty($_POST["penyanggah" . "$i"]) and $_POST["penyanggah" . "$i"] != "") {
										...
										$data_penyanggah = mysqli_query($con, "SELECT `id` FROM `mata_nilai_penyanggah` WHERE `nim`='$penyanggah' AND `dosen`='$_POST[dosen]' AND `presenter`='$_COOKIE[user]' AND `jenis_presentasi`='Kasus Besar'");
								...
								VALUES
								( '$_POST[id]','$penyanggah','$_COOKIE[user]','Kasus Besar','$_POST[tanggal_penyajian]',
									'$judul','$dosen_presentasi[dosen]','$nilai_penyanggah',
									'$tgl','1')");
										}
									}
									$i++;
