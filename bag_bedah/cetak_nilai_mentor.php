...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_mentor = mysqli_query($con, "SELECT * FROM `bedah_nilai_mentor` WHERE `nim`='$nim_mhsw_bedah' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_mentor);
		$halaman = 1;
		$aspek1 = "Persentase Kehadiran";
		$aspek2 = "Pengetahuan Bedah";
		$aspek3 = "Keaktifan";
		$aspek4 = "Keterampilan";
		$aspek5 = "Kerjasama";
		$aspek6 = "Sikap Kesopanan";
		while ($data_mentor = mysqli_fetch_array($daftar_mentor)) {
			//Judul
			$tanggal_mulai = tanggal_indo($data_mentor['tgl_awal']);
			$tanggal_selesai = tanggal_indo($data_mentor['tgl_akhir']);
			$periode = $tanggal_mulai . " s.d. " . $tanggal_selesai;

			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI MENTORING");
			$tabel1[2] = array('item' => "KEPANITERAAN (STASE) ILMU BEDAH");
			...
			$tabel2[3] = array('item' => "Mentoring Bulan Ke-", 'isi' => ": " . "$data_mentor[bulan_ke]");
			$tabel2[4] = array('item' => "Periode Penilaian", 'isi' => ": " . "$periode");
			$pdf->ezTable(
				$tabel2,
				$kolom2,
				"",
			...
			$tabel4[1] = array('NO' => '1', 'ASPEK' => $aspek1, 'NILAI' => $data_mentor['aspek_1']);
			$tabel4[2] = array('NO' => '2', 'ASPEK' => $aspek2, 'NILAI' => $data_mentor['aspek_2']);
			$tabel4[3] = array('NO' => '3', 'ASPEK' => $aspek3, 'NILAI' => $data_mentor['aspek_3']);
			$tabel4[4] = array('NO' => '4', 'ASPEK' => $aspek4, 'NILAI' => $data_mentor['aspek_4']);
			$tabel4[5] = array('NO' => '5', 'ASPEK' => $aspek5, 'NILAI' => $data_mentor['aspek_5']);
			$tabel4[6] = array('NO' => '6', 'ASPEK' => $aspek6, 'NILAI' => $data_mentor['aspek_6']);
			$pdf->ezTable(
				$tabel4,
				$kolom4,
				"",
			...
			$tabel5[1] = array('TOTAL' => 'Rata-Rata Nilai:', 'NILAI' => "<b>$data_mentor[nilai_rata]</b>");
			$pdf->ezTable(
				$tabel5,
				$kolom5,
				"",
			...
			//Kasus Ilmiah
			$kolom5b = array('ITEM' => '');
			$tabel5b[1] = array('ITEM' => "<b>Kasus Ilmiah / Problem Pasien yang Pernah Didiskusikan:</b>\r\n\r\n<i>$data_mentor[kasus]</i>");
			$pdf->ezTable(
				$tabel5b,
				$kolom5b,
				"",
			...
			$tabel6[3] = array('ITEM' => 'Dosen Penilai (Mentor)');
			$tabel6[4] = array('ITEM' => '');
			...
			$pdf->ezSetDy(-10, '');
			$pdf->addText(50, 25, 10, "$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Mentoring di RS Jejaring              <i>[hal $halaman dari $jumlah_halaman hal]</i>");
