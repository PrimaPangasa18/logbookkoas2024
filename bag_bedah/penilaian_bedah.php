...
<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND $_COOKIE['level']==5)
	{
		...
		echo "<center><h4><font style=\"color:#006400;text-shadow:1px 1px black;\">FORM PENILAIAN KEPANITERAAN (STASE) ILMU BEDAH</font></h4><br>";
		$id_stase = "M101";
		...
		if ($cek_stase>=1)
		{
			echo "</center><br><a href=\"#mentor\"><i>Pengisian Formulir Penilaian Mentoring</i></a><br>";
			echo "<a href=\"#test\"><i>Preview Nilai-Nilai Pre-Test, Post-Test, Skill Lab dan OSCE</i></a><br><br>";

			//Pengisian Formulir Penilaian Mentoring
			echo "<a id=\"mentor\"><font style=\"font-size:1.125em;font-weight:bold;font-family:GEORGIA\">Pengisian Formulir Penilaian Mentoring</font></a><br><br>";
			echo "<font style=\"font-size:0.675em\"><i>Catatan:<br>- Pengisian wajib untuk penilaian Mentoring adalah 2 (dua) kali (Penilaian Bulan ke-1 dan Penilaian Bulan ke-2).<br>- Nilai rata-rata adalah jumlah total penilaian dibagi 2 (dua).<br>- Untuk cetak, minimal 1 (satu) penilaian telah disetujui Dosen Penilai (Mentor).</i></font><br><br>";
			$nilai_mentor = mysqli_query($con,"SELECT * FROM `bedah_nilai_mentor` WHERE `nim`='$_COOKIE[user]'");
			...
			else
			{
				...
				while ($data_mentor=mysqli_fetch_array($nilai_mentor))
				{
					...
					if ($data_mentor[status_approval]=='0') echo "<a href=\"approve_mentor.php?id=$data_mentor[id]\"><input type=\"button\" name=\"approve\".\"$data_mentor[id]\" style=\"color:red\" value=\"APPROVE\"></a>";
					echo "</td>";
					echo "<td align=center>";
					if ($data_mentor[status_approval]=='0')
					{
						echo "<font style=\"color:red\">BELUM DISETUJUI</font><br><br>";
						echo "<a href=\"edit_form_mentor.php?id=$data_mentor[id]\"><input type=\"button\" class=\"submit2\" name=\"edit\".\"$data_mentor[id]\" value=\"EDIT\"></a><p>";
						echo "<a href=\"preview_form_mentor.php?id=$data_mentor[id]\"><input type=\"button\" class=\"submit2\" name=\"preview\".\"$data_mentor[id]\" value=\"PREVIEW\"></a><p>";
						echo "<a href=\"hapus_form_mentor.php?id=$data_mentor[id]\"><input type=\"button\" class=\"submit2\" name=\"hapus\".\"$data_mentor[id]\" value=\"HAPUS\"></a>";
					}
					else
					{
						...
						echo "<br><br><a href=\"view_form_mentor.php?id=$data_mentor[id]\"><input type=\"button\" class=\"submit2\" name=\"view\".\"$data_mentor[id]\" value=\"VIEW\"></a><p>";
					}
					echo "</td>";
					echo "</tr>";
					$no++;
			...
			echo "</table>";
			$cek_all_mentor = mysqli_num_rows(mysqli_query($con,"SELECT `id` FROM `bedah_nilai_mentor` WHERE `nim`='$_COOKIE[user]'"));
			$cek_approved_mentor = mysqli_num_rows(mysqli_query($con,"SELECT `id` FROM `bedah_nilai_mentor` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
			if ($cek_all_mentor<2)
			{
			  echo "<br><center><a href=\"tambah_nilai_mentor.php\"><input type=\"button\" class=\"submit1\" name=\"tambah\" value=\"TAMBAH\"></a>";
			  if ($cek_approved_mentor>0)
			  echo "&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"cetak_nilai_mentor.php\" target=\"_BLANK\"><input type=\"button\" class=\"submit1\" name=\"cetak\" value=\"CETAK\"></a>";
			}
			else
			{
			  if ($cek_approved_mentor>0)
			  echo "<br><center><a href=\"cetak_nilai_mentor.php\" target=\"_BLANK\"><input type=\"button\" class=\"submit1\" name=\"cetak\" value=\"CETAK\"></a>";
			}
			echo "</center><br><br>";


			//Preview Nilai-Nilai Pre-Test, Post-Test, Skill Lab dan OSCE
			echo "<a id=\"test\"><font style=\"font-size:1.125em;font-weight:bold;font-family:GEORGIA\">Preview Nilai-Nilai Pre-Test, Post-Test, Skill Lab dan OSCE</font></a><br><br>";
			$nilai_test = mysqli_query($con,"SELECT * FROM `bedah_nilai_test` WHERE `nim`='$_COOKIE[user]'");
			...
			echo "</table>";
			$cek_approved_test = mysqli_num_rows(mysqli_query($con,"SELECT `id` FROM `bedah_nilai_test` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
			...
			echo "<br><br><br><center><a href=\"cetak_nilai_bedah.php\" target=\"_BLANK\"><input type=\"button\" id=\"cetak_nilai\" class=\"submit1\" name=\"cetak_nilai\" value=\"CETAK NILAI\"></a>";
			echo "</center><br><br>";
