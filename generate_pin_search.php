...
<head>
  ...
  <title>On-line Generate OTP Logbook Koas Pendidikan Dokter FK-UNDIP</title>
        ...
        <div class="container-fluid">
          <div class="mb-3">
            ...
            if (isset($_POST['cari']) && $_POST['cari'] === "CARI") {
              ...
              $query = "SELECT * FROM `admin` WHERE (`level`='4' OR `level`='6') AND (`username` LIKE '%$kunci%' OR `nama` LIKE '%$kunci%')";
              $user_kunci = mysqli_query($con, $query);
              $jml = mysqli_num_rows($user_kunci);

              if ($jml >= 1) {
                echo '<div class="text-center mb-4">';
                echo '<small class="text-danger" style="font-weight:700">(Klik tombol username untuk generate OTP dosen)</small>';
                echo '<p></p>';
                echo '<center>';
                ...
                while ($data = mysqli_fetch_array($user_kunci)) {
                  ...
                  echo '<td style="text-align:center"><a href="generate_pin_admin.php?user_name=' . urlencode($data_dosen['nip']) . '" class="btn btn-outline-primary">' . htmlspecialchars($data_dosen['nip']) . '</a></td>';
                ...
                echo '<div class="text-center mb-4">';
                echo '<span style="font-size:1.0em; font-weight:700; color:#dc3545">Tidak ada dosen dengan kata kunci "<span style="color:blue">' . htmlspecialchars($kunci) . '</span>" !!!</span>';
                echo '</div>';
              }
            }
            ?>
