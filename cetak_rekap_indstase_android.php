...
error_reporting('E_ALL ^ E_NOTICE');
		...
		$delete_dummy_penyakit = mysqli_query($con, "DELETE FROM `jurnal_penyakit_dummy` WHERE `username`='$mhsw_nim'");
		while ($data = mysqli_fetch_array($data_jurnal_penyakit)) {
			$insert_penyakit = mysqli_query($con, "INSERT INTO `jurnal_penyakit_dummy`
				...
				VALUES
				('$data[id]', '$data[nim]', '$data_mhsw[nama]', '$data[angkatan]',
				  ...
				  '$data[penyakit4]', '$data[dosen]', '$data[status]', '$data[evaluasi]', '$mhsw_nim')");
		}
		...
		$delete_dummy_ketrampilan = mysqli_query($con, "DELETE FROM `jurnal_ketrampilan_dummy` WHERE `username`='$mhsw_nim'");
		while ($data = mysqli_fetch_array($data_jurnal_ketrampilan)) {
			$insert_ketrampilan = mysqli_query($con, "INSERT INTO `jurnal_ketrampilan_dummy`
				...
				('$data[id]', '$data[nim]', '$data_mhsw[nama]', '$data[angkatan]',
				  ...
				  '$data[ketrampilan4]', '$data[dosen]', '$data[status]', '$data[evaluasi]', '$mhsw_nim')");
		}
			...
			while ($data = mysqli_fetch_array($daftar_penyakit)) {
				if ($cetak == "tengah") {
					if ($data['skdi_level'] == "MKM" or $data['k_level'] == "MKM" or $data['ipsg_level'] == "MKM" or $data['kml_level'] == "MKM") {
				      ...
				      AND `status`='1' AND `evaluasi`='1' AND `username`='$mhsw_nim'"));
						$jml_1 = 0;
						$jml_2 = 0;
						$jml_3 = 0;
						$jml_4A = 0;
