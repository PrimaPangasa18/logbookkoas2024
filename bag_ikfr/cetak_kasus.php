<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_kasus = mysqli_query($con,"SELECT * FROM `ikfr_nilai_kasus` WHERE `nim`='$nim_mhsw_ikfr' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_kasus = mysqli_fetch_array($daftar_kasus))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI DISKUSI KASUS KEPANITERAAN (STASE) IKFR");
			...
			$tabel4{1} = array ('NO'=>'1','ASPEK'=>'Kemampuan Membuat Anamesis','BOBOT'=>'25%','NILAI'=>$data_kasus[aspek_1]);
			$tabel4{2} = array ('NO'=>'2','ASPEK'=>'Kemampuan Menyusun Pemeriksaan Fisik','BOBOT'=>'25%','NILAI'=>$data_kasus[aspek_2]);
			$tabel4{3} = array ('NO'=>'3','ASPEK'=>'Kemampuan Membuat Diagnosis Fungsional','BOBOT'=>'25%','NILAI'=>$data_kasus[aspek_3]);
			$tabel4{4} = array ('NO'=>'4','ASPEK'=>'Kemampuan Membuat Tatalaksana Rehabilitasi Medik','BOBOT'=>'25%','NILAI'=>$data_kasus[aspek_4]);
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Diskusi Kasus                     <i>[hal $halaman dari $jumlah_halaman hal]</i>");
