...
//-------------------------------
$data_nilai_kasus = mysqli_fetch_array(mysqli_query($con, "SELECT `nilai_rata` FROM `ikfr_nilai_kasus` WHERE `nim`='$nim_mhsw_ikfr' AND `status_approval`='1'"));
...
//-------------------------------
$data_nilai_minicex = mysqli_fetch_array(mysqli_query($con, "SELECT `nilai_rata` FROM `ikfr_nilai_minicex` WHERE `nim`='$nim_mhsw_ikfr' AND `status_approval`='1'"));
...
//Rekap Nilai Test
//-------------------------------
$pretest = mysqli_fetch_array(mysqli_query($con, "SELECT MAX(`nilai`) FROM `ikfr_nilai_test` WHERE `nim`='$nim_mhsw_ikfr' AND `jenis_test`='1' AND `status_approval`='1'"));
$posttest = mysqli_fetch_array(mysqli_query($con, "SELECT MAX(`nilai`) FROM `ikfr_nilai_test` WHERE `nim`='$nim_mhsw_ikfr' AND `jenis_test`='2' AND `status_approval`='1'"));
$sikap = mysqli_fetch_array(mysqli_query($con, "SELECT MAX(`nilai`) FROM `ikfr_nilai_test` WHERE `nim`='$nim_mhsw_ikfr' AND `jenis_test`='3' AND `status_approval`='1'"));
$osce = mysqli_fetch_array(mysqli_query($con, "SELECT MAX(`nilai`) FROM `ikfr_nilai_test` WHERE `nim`='$nim_mhsw_ikfr' AND `jenis_test`='5' AND `status_approval`='1'"));
...
//Judul Rekap Total
$data_stase = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM `stase_M105` WHERE `nim`='$nim_mhsw_ikfr'"));
...
$kolom1 = array('item' => "");
$tabel1[1] = array('item' => "REKAP NILAI KEPANITERAAN (STASE) IKFR");
...
$tabel4[1] = array('NO' => '1', 'ITEM' => 'Nilai Diskusi Kasus', 'BOBOT' => '20%', 'NILAI' => $nilai_kasus);
$tabel4[2] = array('NO' => '2', 'ITEM' => 'Nilai Ujian Mini-Cex', 'BOBOT' => '20%', 'NILAI' => $nilai_minicex);
$tabel4[3] = array('NO' => '3', 'ITEM' => "Nilai Ujian OSCE", 'BOBOT' => '20%', 'NILAI' => $nilai_osce);
$tabel4[4] = array('NO' => '4', 'ITEM' => 'Nilai Pre-Test', 'BOBOT' => '10%', 'NILAI' => $nilai_pretest);
$tabel4[5] = array('NO' => '5', 'ITEM' => 'Nilai Post-Test', 'BOBOT' => '20%', 'NILAI' => $nilai_posttest);
$tabel4[6] = array('NO' => '6', 'ITEM' => 'Nilai Sikap dan Perilaku', 'BOBOT' => '10%', 'NILAI' => $nilai_sikap);
$pdf->ezTable(
	$tabel4,
	$kolom4,
	"",
...
$tabel6[1] = array('TOTAL' => 'Nilai Total Kepaniteraan (Stase) IKFR', 'NILAI' => ": " . "<b>$nilai_total</b>");
...
//Persetujuan Kordik
$nip_kordik = mysqli_fetch_array(mysqli_query($con, "SELECT `username` FROM `admin` WHERE `stase`='K094'"));
...
$tabel6a[3] = array('ITEM' => 'Kordik Kepaniteraan (Stase) IKFR');
...
$pdf->ezSetDy(-10, '');
$pdf->addText(50, 25, 10, "$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Rekap Nilai Kepaniteraan (Stase) IKFR           <i>[hal 1]</i>");
$pdf->ezStream(array("Content-Disposition" => "Nilai Bagian IKFR $data_mhsw[nim].pdf"));
