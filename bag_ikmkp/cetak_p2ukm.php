...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_p2ukm = mysqli_query($con, "SELECT * FROM `ikmkp_nilai_p2ukm` WHERE `nim`='$nim_mhsw_ikmkp' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_p2ukm);
		$halaman = 1;
		while ($data_p2ukm = mysqli_fetch_array($daftar_p2ukm)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI KEGIATAN DI P2UKM MLONGGO KEPANITERAAN (STASE) IKM-KP");
			...
			$tabel2[4] = array('item' => "Jenis Kegiatan", 'isi' => ": " . "$data_p2ukm[jenis_penilaian]");
			$pdf->ezTable(
				$tabel2,
				$kolom2,
				"",
			...
			$tabel4[1] = array('NO' => '1', 'ASPEK' => 'Kemampuan penyusunan laporan', 'BOBOT' => '10%', 'NILAI' => $data_p2ukm['aspek_1']);
			$tabel4[2] = array('NO' => '2', 'ASPEK' => 'Kemampuan melakukan presentasi', 'BOBOT' => '10%', 'NILAI' => $data_p2ukm['aspek_2']);
			$tabel4[3] = array('NO' => '3', 'ASPEK' => 'Kemampuan melakukan identifikasi masalah', 'BOBOT' => '20%', 'NILAI' => $data_p2ukm['aspek_3']);
			$tabel4[4] = array('NO' => '4', 'ASPEK' => 'Kemampuan melakukan analisis masalah', 'BOBOT' => '20%', 'NILAI' => $data_p2ukm['aspek_4']);
			$tabel4[5] = array('NO' => '5', 'ASPEK' => 'Kemampuan melakukan intervensi permasalahan', 'BOBOT' => '20%', 'NILAI' => $data_p2ukm['aspek_5']);
			$tabel4[6] = array('NO' => '6', 'ASPEK' => 'Kelancaran diskusi dan tanya jawab', 'BOBOT' => '10%', 'NILAI' => $data_p2ukm['aspek_6']);
			$tabel4[7] = array('NO' => '7', 'ASPEK' => 'Sikap', 'BOBOT' => '10%', 'NILAI' => $data_p2ukm['aspek_7']);
			$pdf->ezTable(
				$tabel4,
				$kolom4,
				"",
			...
			$tabel5[1] = array('TOTAL' => 'Total Nilai (Bobot x Nilai):', 'NILAI' => $data_p2ukm['nilai_total']);
			$pdf->ezTable(
				$tabel5,
				$kolom5,
				"",
			...
			$pdf->ezSetDy(-10, '');
			$pdf->addText(50, 25, 10, "$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Kegiatan di P2UKM Mlonggo    <i>[hal $halaman dari $jumlah_halaman hal]</i>");
