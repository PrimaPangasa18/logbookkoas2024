...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_komprehensip = mysqli_query($con, "SELECT * FROM `ikmkp_nilai_komprehensip` WHERE `nim`='$nim_mhsw_ikmkp' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_komprehensip);
		$halaman = 1;
		while ($data_komprehensip = mysqli_fetch_array($daftar_komprehensip)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI UJIAN KOMPREHENSIP KEPANITERAAN (STASE) IKM-KP");
			...
			$tabel4[1] = array('NO' => '1', 'ASPEK' => 'Penilaian portofolio kegiatan mahasiwa selama kepaniteraan IKM-KP (kelengkapan laporan, kualitas laporan)', 'BOBOT' => '20%', 'NILAI' => $data_komprehensip['aspek_1']);
			$tabel4[2] = array('NO' => '2', 'ASPEK' => 'Kemampuan ujian lisan (kemampuan menjawab pertanyaan, penguasaan materi, kemampuan melakukan reasoning dan analisis)', 'BOBOT' => '50%', 'NILAI' => $data_komprehensip['aspek_2']);
			$tabel4[3] = array('NO' => '3', 'ASPEK' => 'Kemampuan ujian tertulis/penugasan', 'BOBOT' => '20%', 'NILAI' => $data_komprehensip['aspek_3']);
			$tabel4[4] = array('NO' => '4', 'ASPEK' => 'Sikap', 'BOBOT' => '10%', 'NILAI' => $data_komprehensip['aspek_4']);
			$pdf->ezTable(
				$tabel4,
				$kolom4,
				"",
			...
			$tabel5[1] = array('TOTAL' => 'Total Nilai (Bobot x Nilai):', 'NILAI' => $data_komprehensip['nilai_total']);
			$pdf->ezTable(
				$tabel5,
				$kolom5,
				"",
			...
			$pdf->ezSetDy(-10, '');
			$pdf->addText(50, 25, 10, "$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Ujian Komprehensip             <i>[hal $halaman dari $jumlah_halaman hal]</i>");
