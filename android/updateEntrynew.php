...
$query_check = "SELECT * FROM evaluasi WHERE nim='$username' AND grup='$grup' AND stase='$stase' AND tanggal='$tanggal'";
$check_result = mysqli_query($conn, $query_check);

if ($check_result && mysqli_num_rows($check_result) > 0) {
    // Record exists, perform update
    $query_update_evaluasi = "
        UPDATE evaluasi 
        SET evaluasi='$SEvaluasi', rencana='$SRencana' 
        WHERE nim='$username' AND grup='$grup' AND stase='$stase' AND tanggal='$tanggal' AND angkatan='$angkatan'
    ";
    ...
    $query_insert_evaluasi = "
        INSERT INTO evaluasi (nim, grup, stase, tanggal, evaluasi, rencana, angkatan) 
        VALUES ('$username', '$grup', '$stase', '$tanggal', '$SEvaluasi', '$SRencana', '$angkatan')
    ";
    if (!mysqli_query($conn, $query_insert_evaluasi)) {
        $response["status"] = 1;
        $response["message"] = "Error inserting into evaluasi: " . mysqli_error($conn);
        echo json_encode($response);
        exit();
    }
}
...
$query_jp = "UPDATE jurnal_penyakit SET evaluasi=1 WHERE stase='".$stase."' AND nim='".$username."'  AND stase='".$stase."' AND tanggal='".$tanggal."'";
...
$query_jk = "UPDATE jurnal_ketrampilan SET evaluasi=1 WHERE stase='".$stase."' AND nim='".$username."' AND stase='".$stase."' AND tanggal='".$tanggal."'";
