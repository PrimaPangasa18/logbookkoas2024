<?php
include 'connect.php';
include "fungsi_php7.php";
$inputJSON = file_get_contents('php://input');
$input = json_decode($inputJSON, TRUE);
	$nim = $input["username"];
	$nama = $input["nama"];
    $id_stase = $input["bagian"];
	
    $stase_id = "stase_" . $id_stase;
    $include_id = "include_" . $id_stase;
    $target_id = "target_" . $id_stase;

    $data_mhsw = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `biodata_mhsw` WHERE `nim`='".$nim."'"));
    $data_stase = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM `$stase_id` WHERE `nim`='".$nim."'"));
    ...
    while ($data = mysqli_fetch_array($jurnal_penyakit)) {
        mysqli_query($conn, "INSERT INTO `jurnal_penyakit_dummy`
    ...
    while ($data = mysqli_fetch_array($jurnal_ketrampilan)) {
        mysqli_query($conn, "INSERT INTO `jurnal_ketrampilan_dummy`
            (`id`, `nim`, `nama`, `angkatan`, `grup`,
             `hari`, `tanggal`, `stase`,
             `jam_awal`, `jam_akhir`, `kelas`,
             `lokasi`, `kegiatan`,
    ...
    //--------------------------------------------------------------------
            ...
            if ($id_stase=="M121")
			{
                ...
                'nilai_kompre' => $nilai_kompre,
                'nilai_kdk' => $nilai_kdk,
                'nilai_bagian' => $nilai_bagian,
            ];
            } else {
                $response = [
                'periode' => $periode,
