<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_jurnal = mysqli_query($con,"SELECT * FROM `ika_nilai_jurnal` WHERE `nim`='$nim_mhsw_ika' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_jurnal = mysqli_fetch_array($daftar_jurnal))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI PENYAJIAN JOURNAL READING");
			...
			$tabel2{5} = array ('item'=>"Judul Paper/Makalah", 'isi'=>": "."$data_jurnal[jurnal]");
			...
			$tabel4b{1} = array ('NO'=>'1','ASPEK'=>'Slide dan audiovisual','BOBOT'=>'5%','NILAI'=>$data_jurnal[aspek_1]);
			$tabel4b{2} = array ('NO'=>'2','ASPEK'=>'Sikap dan suara','BOBOT'=>'15%','NILAI'=>$data_jurnal[aspek_2]);
			...
			$tabel4d{1} = array ('NO'=>'3','ASPEK'=>'Kesesuaian format penulisan','BOBOT'=>'10%','NILAI'=>$data_jurnal[aspek_3]);
			$tabel4d{2} = array ('NO'=>'4','ASPEK'=>'Pendahuluan dan Metodologi','BOBOT'=>'10%','NILAI'=>$data_jurnal[aspek_4]);
			$tabel4d{3} = array ('NO'=>'5','ASPEK'=>'Diskusi hasil penelitian','BOBOT'=>'10%','NILAI'=>$data_jurnal[aspek_5]);
			$tabel4d{4} = array ('NO'=>'6','ASPEK'=>'Penerapan EBM','BOBOT'=>'10%','NILAI'=>$data_jurnal[aspek_6]);
			$tabel4d{5} = array ('NO'=>'7','ASPEK'=>'Pengambilan kesimpulan','BOBOT'=>'10%','NILAI'=>$data_jurnal[aspek_7]);
			...
			$tabel4f{1} = array ('NO'=>'8','ASPEK'=>"Penalaran dan penguasaan materi",'BOBOT'=>'20%','NILAI'=>$data_jurnal[aspek_8]);
			$tabel4f{2} = array ('NO'=>'9','ASPEK'=>"Cara mempertahankan argumentasi",'BOBOT'=>'10%','NILAI'=>$data_jurnal[aspek_9]);
			...
			$tabel5{1} = array ('TOTAL'=>'Rata-Rata Nilai (Jumlah Bobot x Nilai)','NILAI'=>"<b>$data_jurnal[nilai_rata]</b>");
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Penyajian Journal Reading        <i>[hal $halaman dari $jumlah_halaman hal]</i>");
