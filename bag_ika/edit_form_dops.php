...
<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND $_COOKIE['level']==5)
	{
		...
		echo "<center><h4 id=\"top\"><font style=\"color:#006400;text-shadow:1px 1px black;\">EDIT NILAI DIRECT OBSERVATION OF PROCEDURAL SKILL (DOPS)<br>(PENILAIAN KETRAMPILAN KLINIS)<p>KEPANITERAAN (STASE) ILMU KESEHATAN ANAK</font></h4>";
		...
		if ($_POST[ubah]=="UBAH")
		{
			...
			$update_dops=mysqli_query($con,"UPDATE `ika_nilai_dops` SET
				`dosen`='$_POST[dosen]',`situasi_ruangan`='$_POST[situasi_ruangan]',`tindak_medik`='$tindak_medik',
				...
				`tgl_isi`='$tgl',`tgl_ujian`='$_POST[tanggal_ujian]'
				WHERE `id`='$_POST[id]'");
