...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		while ($data_kasus = mysqli_fetch_array($daftar_kasus)) {
			...
			$kolom1 = array('item' => "");
			$tabel1{
			1} = array('item' => "NILAI PENYAJIAN KASUS BESAR");
			$tabel1{
			2} = array('item' => "KEPANITERAAN (STASE) ILMU KESEHATAN ANAK");
			$tabel1{
			3} = array('item' => "LOGBOOK KOAS PENDIDIKAN PROFESI DOKTER");
			$tabel1{
			4} = array('item' => "FAKULTAS KEDOKTERAN - UNIVERSITAS DIPONEGORO");
			$pdf->ezTable(
				$tabel1,
				$kolom1,
				"",
			...
			$tabel2{
			3} = array('item' => "Kepaniteraan (Stase)", 'isi' => ": " . "Ilmu Kesehatan Anak");
			$tabel2{
			4} = array('item' => "Periode Stase", 'isi' => ": " . "$periode_stase");
			$tabel2{
			5} = array('item' => "Judul Kasus", 'isi' => ": " . "$data_kasus[kasus]");
			$tabel2{
			6} = array('item' => "Tanggal Penyajian", 'isi' => ": " . "$tanggal_penyajian");
			$pdf->ezTable(
				$tabel2,
				$kolom2,
				"",
			...
			$tabel3{
			1} = array('NO' => '<b>No</b>', 'ASPEK' => '<b>Komponen Penilaian</b>', 'BOBOT' => '<b>Bobot</b>', 'NILAI' => '<b>Nilai (0-100)</b>');
			$pdf->ezTable(
				$tabel3,
				$kolom3,
				"",
			...
			$kolom4a = array('ASPEK' => '');
			$tabel4a{
			1} = array('ASPEK' => "<b>Aspek Penyajian:</b>");
			$pdf->ezTable(
				$tabel4a,
				$kolom4a,
				"",
			...
			$tabel4b{
			1} = array('NO' => '1', 'ASPEK' => 'Slide', 'BOBOT' => '5%', 'NILAI' => $data_kasus[aspek_1]);
			$tabel4b{
			2} = array('NO' => '2', 'ASPEK' => 'Sikap dan suara', 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_2]);
			$pdf->ezTable(
				$tabel4b,
				$kolom4b,
				"",
			...
			$kolom4c = array('ASPEK' => '');
			$tabel4c{
			1} = array('ASPEK' => "<b>Aspek Naskah:</b>");
			$pdf->ezTable(
				$tabel4c,
				$kolom4c,
				"",
			...
			$tabel4d{
			1} = array('NO' => '3', 'ASPEK' => 'Kesesuaian format penulisan', 'BOBOT' => '2.5%', 'NILAI' => $data_kasus[aspek_3]);
			$tabel4d{
			2} = array('NO' => '4', 'ASPEK' => 'Ringkasan kasus', 'BOBOT' => '5%', 'NILAI' => $data_kasus[aspek_4]);
			$tabel4d{
			3} = array('NO' => '5', 'ASPEK' => 'Pembahasan dan aplikasi EBM (PICO-VIA)', 'BOBOT' => '5%', 'NILAI' => $data_kasus[aspek_5]);
			$tabel4d{
			4} = array('NO' => '6', 'ASPEK' => 'Kepustakaan', 'BOBOT' => '2.5%', 'NILAI' => $data_kasus[aspek_6]);
			$pdf->ezTable(
				$tabel4d,
				$kolom4d,
				"",
			...
			$kolom4e = array('ASPEK' => '');
			$tabel4e{
			1} = array('ASPEK' => "<b>Aspek Diskusi:</b>");
			$pdf->ezTable(
				$tabel4e,
				$kolom4e,
				"",
			...
			$tabel4f{
			1} = array('NO' => '7', 'ASPEK' => "Anamnesis (<i>sacred seven, fundamental four</i>)", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_7]);
			$tabel4f{
			2} = array('NO' => '8', 'ASPEK' => "Pemeriksaan Fisik (<i>status lokalis dan generalis</i>)", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_8]);
			$tabel4f{
			3} = array('NO' => '9', 'ASPEK' => "Pemeriksaan Penunjang (<i>jenis dan interpretasi hasil</i>)", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_9]);
			$tabel4f{
			4} = array('NO' => '10', 'ASPEK' => "Diagnosis Banding/Diagnosis Kerja", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_10]);
			$tabel4f{
			5} = array('NO' => '11', 'ASPEK' => "Tatalaksana", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_11]);
			$tabel4f{
			6} = array('NO' => '12', 'ASPEK' => "Komplikasi, Pencegahan, Prognosis", 'BOBOT' => '5%', 'NILAI' => $data_kasus[aspek_12]);
			$tabel4f{
			7} = array('NO' => '13', 'ASPEK' => "Tumbuh kembang (<i>nutrisi, imunisasi, perkembangan</i>)", 'BOBOT' => '10%', 'NILAI' => $data_kasus[aspek_13]);
			$tabel4f{
			8} = array('NO' => '14', 'ASPEK' => "Diskusi lain", 'BOBOT' => '5%', 'NILAI' => $data_kasus[aspek_14]);
			$pdf->ezTable(
				$tabel4f,
				$kolom4f,
				"",
			...
			$tabel5{
			1} = array('TOTAL' => 'Rata-Rata Nilai (Jumlah Bobot x Nilai)', 'NILAI' => "<b>$data_kasus[nilai_rata]</b>");
			$pdf->ezTable(
				$tabel5,
				$kolom5,
				"",
