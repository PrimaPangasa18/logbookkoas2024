...
<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND $_COOKIE['level']==4)
	{
		...
		echo "<center><h4 id=\"top\"><font style=\"color:#006400;text-shadow:1px 1px black;\">APPROVAL NILAI UJIAN MINI-CEX - UJIAN KOMPETENSI KLINIK<p>Kepaniteraan (Stase) Ilmu Kesehatan Jiwa</font></h4>";
		...
		if ($_POST[approve]=="APPROVE")
		{
			...
			$update_minicex=mysqli_query($con,"UPDATE `psikiatri_nilai_minicex` SET
				...
				`kepuasan1`='$_POST[kepuasan1]',`tgl_approval`='$tgl',`status_approval`='1'
				WHERE `id`='$_POST[id]'");
