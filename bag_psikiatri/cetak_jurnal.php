<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$datastase_mhsw = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `stase_M093` WHERE `nim`='$nim_mhsw_psikiatri'"));
		...
		$daftar_jurnal = mysqli_query($con,"SELECT * FROM `psikiatri_nilai_jurnal` WHERE `nim`='$nim_mhsw_psikiatri' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_jurnal = mysqli_fetch_array($daftar_jurnal))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI JOURNAL READING KEPANITERAAN (STASE) ILMU KESEHATAN JIWA");
			...
			//Catatan
			$kolom5a = array ('ITEM'=>'');
			$tabel5a{1} = array ('ITEM'=>"<b>Catatan Dosen Penguji:</b>\r\n\r\n<i>$data_jurnal[catatan]</i>");
