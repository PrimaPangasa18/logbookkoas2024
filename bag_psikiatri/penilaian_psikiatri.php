...
<head>
	...
	<title>On-line Penilaian Bagian Ilmu Kesehatan Jiwa Logbook Koas Pendidikan Dokter FK-UNDIP</title>
			...
			<main class="content px-3 py-4" style="background-image: url('images/undip_watermark_color.png'), url('images/undip_watermark_color.png'); ">
				<div class="container-fluid">
					<div class="mb-3">
						...
						<br>
						<h2 class="fw-bold fs-4 mb-3 text-center" style="color:#0A3967">FORM PENILAIAN KEPANITERAAN (STASE) ILMU KESEHATAN JIWA</h2>
						<br>
						<?php
						$id_stase = "M093";
						...
						if ($cek_stase >= 1) {
							echo "
								...
								<br><br>
								<a href=\"#jurnal\" class=\"btn btn-success me-3\">Pengisian Formulir Penilaian Journal Reading</a>
								<a href=\"#cbd\" class=\"btn btn-primary me-3\">Pengisian Formulir Penilaian CBD</a>
								<a href=\"#minicex\" class=\"btn me-3\" style=\"background-color: #800080; color: white;\">Pengisian Formulir Penilaian Ujian MINI-CEX</a>
								<br><br>
								<a href=\"#osce\" class=\"btn me-3\" style=\"background-color: #A52A2A; color: white;\">Pengisian Formulir Penilaian Ujian OSCE</a>
							...
							$nilai_jurnal = mysqli_query($con, "SELECT * FROM `psikiatri_nilai_jurnal` WHERE `nim`='$_COOKIE[user]'");
							...
							$cek_approved_jurnal = mysqli_num_rows(mysqli_query($con, "SELECT `id` FROM `psikiatri_nilai_jurnal` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
							...
							echo "<a id=\"cbd\" style=\"font-size:1.2em;font-family:'Poppins', sans-serif;font-weight:800;\">Pengisian Formulir Penilaian CBD</a><br><br>";
							$nilai_cbd = mysqli_query($con, "SELECT * FROM `psikiatri_nilai_cbd` WHERE `nim`='$_COOKIE[user]'");
							...
							$cek_approved_cbd = mysqli_num_rows(mysqli_query($con, "SELECT `id` FROM `psikiatri_nilai_cbd` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
							if ($cek_approved_cbd > 0)
								echo "<br><center><a href=\"cetak_cbd.php\" target=\"_BLANK\">
								<button type=\"button\" style=\"margin-right: 10px;\" class=\"btn btn-primary\" name=\"cetak\" value=\"CETAK\"><i class=\"fas fa-print\"></i> CETAK</button>
								</a>";
							echo "</center><br><br>";
							echo "<hr style=\"border: 2px solid ; color:blue; margin: 20px 0;\">";
							...
							$nilai_minicex = mysqli_query($con, "SELECT * FROM `psikiatri_nilai_minicex` WHERE `nim`='$_COOKIE[user]'");
							...
							$cek_approved_minicex = mysqli_num_rows(mysqli_query($con, "SELECT `id` FROM `psikiatri_nilai_minicex` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
							...
							echo "<a id=\"osce\" style=\"font-size:1.2em;font-family:'Poppins', sans-serif;font-weight:800;\">Pengisian Formulir Penilaian Ujian OSCE</a><br><br>";
							$nilai_osce = mysqli_query($con, "SELECT * FROM `psikiatri_nilai_osce` WHERE `nim`='$_COOKIE[user]'");
							...
							if ($cek_nilai_osce < 1) {
								...
								while ($data_osce = mysqli_fetch_array($nilai_osce)) {
									...
									if ($data_osce['status_approval'] == '0')
										echo " <a href=\"approve_osce.php?id=$data_osce[id]\">
   												<button type=\"button\"  class=\"btn btn-primary btn-sm\" name=\"approve_$data_osce[id]\" >
    											<i class=\"fas fa-notes-medical me-2\"></i> APPROVE
												</button></a>";
									echo "</td>";
									echo "<td align=center>";
									if ($data_osce['status_approval'] == '0') {
										echo "<font style=\"color:red; font-weight:600;\">BELUM DISETUJUI</font><br><br>";
										echo "<a href=\"edit_form_osce.php?id=$data_osce[id]\">
       										 <button type=\"button\" class=\"btn btn-warning btn-sm\" name=\"edit_$data_osce[id]\" value=\"EDIT\">
            								<i class=\"fas fa-edit me-2\"></i> EDIT
       										</button>
      										</a><p><br>";
										echo "<a href=\"preview_form_osce.php?id=$data_osce[id]\">
        										<button type=\"button\" class=\"btn btn-primary btn-sm\" name=\"preview_$data_osce[id]\" value=\"PREVIEW\">
            								<i class=\"fas fa-eye me-2\"></i> PREVIEW
        									</button>
      										</a><p>";
										echo "<a href=\"hapus_form_osce.php?id=$data_osce[id]\">
        									<button type=\"button\" class=\"btn btn-danger btn-sm\" name=\"hapus_$data_osce[id]\" value=\"HAPUS\">
            								<i class=\"fas fa-trash-alt me-2\"></i> HAPUS
        									</button>
      										</a>";
									} else {
										...
										echo "<span style=\"color:darkblue; font-weight:600;\">$tanggal_approval</span>";
										echo "<br><br>
										<a href=\"view_form_osce.php?id=$data_osce[id]\">
							...
							echo "</table>";
							if ($cek_nilai_osce < 1)
								echo "<br><center><a href=\"tambah_osce.php\" >
								<button type=\"button\" style=\"margin-right: 10px;\" class=\"btn btn-success\" name=\"tambah\" value=\"TAMBAH\"><i class=\"fas fa-plus\"></i> TAMBAH</button>
								</a>";
							$cek_approved_osce = mysqli_num_rows(mysqli_query($con, "SELECT `id` FROM `psikiatri_nilai_osce` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
							if ($cek_approved_osce > 0)
								echo "<br><center><a href=\"cetak_osce.php\" target=\"_BLANK\">
								<button type=\"button\" style=\"margin-right: 10px;\" class=\"btn btn-primary\" name=\"cetak\" value=\"CETAK\"><i class=\"fas fa-print\"></i> CETAK</button>
								</a>";
							echo "</center><br><br>";
							echo "<hr style=\"border: 2px solid ; color:blue; margin: 20px 0;\">";
							...
							$nilai_test = mysqli_query($con, "SELECT * FROM `psikiatri_nilai_test` WHERE `nim`='$_COOKIE[user]'");
							...
							echo "<center>";
							echo "<br>";
							$cek_approved_test = mysqli_num_rows(mysqli_query($con, "SELECT `id` FROM `psikiatri_nilai_test` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
							if ($cek_approved_test > 0)
								echo "<br><center><a href=\"cetak_nilai_test.php\" target=\"_BLANK\">
								<button type=\"button\" style=\"margin-right: 10px;\" class=\"btn btn-primary\" name=\"cetak\" value=\"CETAK\"><i class=\"fas fa-print\"></i> CETAK</button>
								</a>";
							echo "<br><br><br><a href=\"cetak_nilai_psikiatri.php\" target=\"_BLANK\">";
