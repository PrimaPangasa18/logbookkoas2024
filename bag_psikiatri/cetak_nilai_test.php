<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_test = mysqli_query($con,"SELECT * FROM `psikiatri_nilai_test` WHERE `nim`='$nim_mhsw_psikiatri' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_test = mysqli_fetch_array($daftar_test))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI TEST KEPANITERAAN (STASE) ILMU KESEHATAN JIWA");
			...
			$tabel6{3} = array ('ITEM'=>'Kordik Kepaniteraan (Stase) Ilmu Kesehatan Jiwa');
			$tabel6{4} = array ('ITEM'=>'');
