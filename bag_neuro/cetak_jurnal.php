...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_jurnal = mysqli_query($con, "SELECT * FROM `neuro_nilai_jurnal` WHERE `nim`='$nim_mhsw_neuro' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_jurnal);
		$halaman = 1;
		while ($data_jurnal = mysqli_fetch_array($daftar_jurnal)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI JOURNAL READING KEPANITERAAN (STASE) NEUROLOGI");
