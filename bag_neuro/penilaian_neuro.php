...
<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND $_COOKIE['level']==5)
	{
		...
		echo "<center><h4><font style=\"color:#006400;text-shadow:1px 1px black;\">FORM PENILAIAN KEPANITERAAN (STASE) NEUROLOGI</font></h4><br>";
		$id_stase = "M092";
		...
		if ($cek_stase>=1)
		{
			echo "</center><br><a href=\"#cbd\"><i>Pengisian Formulir Penilaian Kasus CBD</i></a><br>";
			echo "<a href=\"#jurnal\"><i>Pengisian Formulir Penilaian Journal Reading</i></a><br>";
			echo "<a href=\"#spv\"><i>Pengisian Formulir Penilaian Ujian SPV</i></a><br>";
			echo "<a href=\"#minicex\"><i>Pengisian Formulir Penilaian Ujian Minicex</i></a><br>";
			echo "<a href=\"#test\"><i>Preview Nilai Penugasan dan Test</i></a><br><br>";

			//Pengisian Formulir Penilaian Kasus CBD
			echo "<a id=\"cbd\"><font style=\"font-size:1.125em;font-weight:bold;font-family:GEORGIA\">Pengisian Formulir Penilaian Kasus CBD</font></a><br><br>";
			$nilai_cbd = mysqli_query($con,"SELECT * FROM `neuro_nilai_cbd` WHERE `nim`='$_COOKIE[user]'");
			...
			$cek_approved_cbd = mysqli_num_rows(mysqli_query($con,"SELECT `id` FROM `neuro_nilai_cbd` WHERE `nim`='$_COOKIE[user]' AND `status_approval`='1'"));
			...
			$nilai_jurnal = mysqli_query($con,"SELECT * FROM `neuro_nilai_jurnal` WHERE `nim`='$_COOKIE[user]'");
			...
			echo "<a id=\"spv\"><font style=\"font-size:1.125em;font-weight:bold;font-family:GEORGIA\">Pengisian Formulir Penilaian Ujian SPV</font></a><br><br>";
			$nilai_spv = mysqli_query($con,"SELECT * FROM `neuro_nilai_spv` WHERE `nim`='$_COOKIE[user]'");
			...
			else
			{
			  ...
			  while ($data_spv=mysqli_fetch_array($nilai_spv))
			  {
			    ...
			    echo "<td>$data_dosen[nama], $data_dosen[gelar] (NIP. $data_dosen[nip])<br><br>";
			    if ($data_spv[status_approval]=='0') echo "<a href=\"approve_spv.php?id=$data_spv[id]\"><input type=\"button\" name=\"approve\".\"$data_spv[id]\" style=\"color:red\" value=\"APPROVE\"></a>";
			    echo "</td>";
			    echo "<td align=center>";
			    if ($data_spv[status_approval]=='0')
			    {
			      echo "<font style=\"color:red\">BELUM DISETUJUI</font><br><br>";
			      echo "<a href=\"edit_form_spv.php?id=$data_spv[id]\"><input type=\"button\" class=\"submit2\" name=\"edit\".\"$data_spv[id]\" value=\"EDIT\"></a><p>";
			      echo "<a href=\"preview_form_spv.php?id=$data_spv[id]\"><input type=\"button\" class=\"submit2\" name=\"preview\".\"$data_spv[id]\" value=\"PREVIEW\"></a><p>";
			      echo "<a href=\"hapus_form_spv.php?id=$data_spv[id]\"><input type=\"button\" class=\"submit2\" name=\"hapus\".\"$data_spv[id]\" value=\"HAPUS\"></a>";
			    }
			    else
			    {
			      echo "<font style=\"color:green\">DISETUJUI</font><br>";
			      echo "per tanggal<br>";
			      echo "$tanggal_approval";
			      echo "<br><br><a href=\"view_form_spv.php?id=$data_spv[id]\"><input type=\"button\" class=\"submit2\" name=\"view\".\"$data_spv[id]\" value=\"VIEW\"></a><p>";
			    }
			    echo "</td>";
			    echo "</tr>";
			    $no++;
			...
			echo "</table>";
			if ($cek_nilai_spv<1)
			echo "<br><center><a href=\"tambah_spv.php\"><input type=\"button\" class=\"submit1\" name=\"tambah\" value=\"TAMBAH\"></a>";
			...
			echo "<br><center><a href=\"cetak_spv.php\" target=\"_BLANK\"><input type=\"button\" class=\"submit1\" name=\"cetak\" value=\"CETAK\"></a>";
			echo "</center><br><br>";
			...
			$nilai_minicex = mysqli_query($con,"SELECT * FROM `neuro_nilai_minicex` WHERE `nim`='$_COOKIE[user]'");
			...
			echo "<a id=\"test\"><font style=\"font-size:1.125em;font-weight:bold;font-family:GEORGIA\">Preview Nilai Ujian/Test</font></a><br><br>";
			$nilai_test = mysqli_query($con,"SELECT * FROM `neuro_nilai_test` WHERE `nim`='$_COOKIE[user]'");
			...
			echo "<br><br><br><center><a href=\"cetak_nilai_neuro.php\" target=\"_BLANK\"><input type=\"button\" id=\"cetak_nilai\" class=\"submit1\" name=\"cetak_nilai\" value=\"CETAK NILAI\"></a>";
			echo "</center><br><br>";
