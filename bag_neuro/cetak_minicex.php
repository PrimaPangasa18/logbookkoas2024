...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_minicex = mysqli_query($con, "SELECT * FROM `neuro_nilai_minicex` WHERE `nim`='$nim_mhsw_neuro' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_minicex);
		$halaman = 1;
		while ($data_minicex = mysqli_fetch_array($daftar_minicex)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI UJIAN MINI-CEX KEPANITERAAN (STASE) NEUROLOGI");
			...
			$tabel2[6] = array('item' => "Situasi Ruangan", 'isi' => ": " . "$data_minicex[situasi_ruangan]");
			$tabel2[7] = array('item' => "Umur Pasien", 'isi' => ": " . "$data_minicex[umur_pasien]" . " tahun");
			$tabel2[8] = array('item' => "Jenis Kelamin Pasien", 'isi' => ": " . "$data_minicex[jk_pasien]");
			$tabel2[9] = array('item' => "Problem/Diagnosis Pasien", 'isi' => ": " . "$data_minicex[diagnosis]");
			$tabel2[10] = array('item' => "Tingkat Kerumitan", 'isi' => ": " . "$data_minicex[tingkat_kerumitan]");
