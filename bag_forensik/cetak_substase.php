<?php
	...
	{
		if ($_COOKIE[level]=='5') $nim_mhsw_forensik = $_COOKIE[user];
		if ($_COOKIE[level]=='1' OR $_COOKIE[level]=='2' OR $_COOKIE[level]=='3') $nim_mhsw_forensik = $_GET[nim];
		...
		$daftar_substase = mysqli_query($con,"SELECT * FROM `forensik_nilai_substase` WHERE `nim`='$nim_mhsw_forensik' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_substase);
		$halaman = 1;
		while ($data_substase = mysqli_fetch_array($daftar_substase))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI SUBSTASE KEPANITERAAN (STASE) KEDOKTERAN FORENSIK DAN MEDIKOLEGAL");
			...
			$tabel2{4} = array ('item'=>"Dosen Penilai Ke-", 'isi'=>": "."$data_substase[dosen_ke]");
			...
			//Nilai Aspek 1 - 5
			$kehadiran = "Kehadiran:\r\n<i>-	Tepat waktu = 100\r\n- Terlambat < 5 menit = 90\r\n- Terlambat 5 - 10 menit = 80\r\n-	Terlambat 10 - 15 menit = 70\r\n-	Terlambat > 15 menit = 50\r\n- Tidak hadir = 0</i>";
			$kolom4 = array ('NO'=>'','ASPEK'=>'','BOBOT'=>'','NILAI'=>'');
			$tabel4{1} = array ('NO'=>'1','ASPEK'=>"$kehadiran",'BOBOT'=>'30%','NILAI'=>$data_substase[aspek_1]);
			$tabel4{2} = array ('NO'=>'2','ASPEK'=>'Keaktifan bertanya','BOBOT'=>'30%','NILAI'=>$data_substase[aspek_2]);
			$tabel4{3} = array ('NO'=>'3','ASPEK'=>'Keaktifan menjawab dengan benar','BOBOT'=>'40%','NILAI'=>$data_substase[aspek_3]);
			...
			$tabel5{1} = array ('TOTAL'=>'Rata-Rata Nilai (Jumlah Bobot x Nilai)','NILAI'=>"<b>$data_substase[nilai_rata]</b>");
			...
			$tabel5a{2} = array ('ITEM'=>"Catatan:\r\n<i>$data_substase[catatan]</i>");
			$tabel5a{3} = array ('ITEM'=>"Saran:\r\n<i>$data_substase[saran]</i>");
			$pdf->ezTable($tabel5a,$kolom5a,"",
			array('maxWidth'=>530,'width'=>510,'fontSize'=>10,'showHeadings'=>0,'shaded'=>0,'showLines'=>1,'titleFontSize'=>12, 'xPos' => 'center','xOrientation' => 'center','rowGap' => 3 ,'showBgCol'=> 0,
			'cols'=>array('TOTAL'=>array('justification'=>'right'),'NILAI'=>array('width'=>100,'justification'=>'center'))));
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Substase                                  <i>[hal $halaman dari $jumlah_halaman hal]</i>");
