<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_jaga = mysqli_query($con,"SELECT * FROM `forensik_nilai_jaga` WHERE `nim`='$nim_mhsw_forensik' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_jaga);
		$halaman = 1;
		while ($data_jaga = mysqli_fetch_array($daftar_jaga))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI KEGIATAN JAGA KEPANITERAAN (STASE) KEDOKTERAN FORENSIK DAN MEDIKOLEGAL");
			...
			$tabel2{4} = array ('item'=>"Dosen Penilai Ke-", 'isi'=>": "."$data_jaga[dosen_ke]");
			...
			//Nilai Aspek 1 - 5
			$kehadiran = "Kehadiran:\r\n<i>-	Tepat waktu = 100\r\n- Terlambat < 5 menit = 90\r\n- Terlambat 5 - 10 menit = 80\r\n-	Terlambat 10 - 15 menit = 70\r\n-	Terlambat > 15 menit = 50\r\n- Tidak hadir = 0\r\nCtt: Hadir tetapi tidak berada di tempat saat pengecekan oleh residen tanpa keterangan, nilai dikurangi 30</i>";
			$kolom4 = array ('NO'=>'','ASPEK'=>'','BOBOT'=>'','NILAI'=>'');
			$tabel4{1} = array ('NO'=>'1','ASPEK'=>"$kehadiran",'BOBOT'=>'30%','NILAI'=>$data_jaga[aspek_1]);
			$tabel4{2} = array ('NO'=>'2','ASPEK'=>'Keaktifan bertanya / berdiskusi / berlatih ketrampilan','BOBOT'=>'70%','NILAI'=>$data_jaga[aspek_2]);
			...
			$tabel5{1} = array ('TOTAL'=>'Rata-Rata Nilai (Jumlah Bobot x Nilai)','NILAI'=>"<b>$data_jaga[nilai_rata]</b>");
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Kegiatan Jaga                     <i>[hal $halaman dari $jumlah_halaman hal]</i>");
