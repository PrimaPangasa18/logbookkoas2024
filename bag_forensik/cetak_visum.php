<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_visum = mysqli_query($con,"SELECT * FROM `forensik_nilai_visum` WHERE `nim`='$nim_mhsw_forensik' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_visum);
		$halaman = 1;
		while ($data_visum = mysqli_fetch_array($daftar_visum))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI VISUM BAYANGAN KEPANITERAAN (STASE) KEDOKTERAN FORENSIK DAN MEDIKOLEGAL");
			...
			$tabel2{4} = array ('item'=>"Dosen Penilai Ke-", 'isi'=>": "."$data_visum[dosen_ke]");
			...
			$tabel4{1} = array ('NO'=>'1','ASPEK'=>"$kehadiran",'BOBOT'=>'30%','NILAI'=>$data_visum[aspek_1]);
			$tabel4{2} = array ('NO'=>'2','ASPEK'=>'Keaktifan bertanya','BOBOT'=>'30%','NILAI'=>$data_visum[aspek_2]);
			$tabel4{3} = array ('NO'=>'3','ASPEK'=>'Keaktifan menjawab dengan benar','BOBOT'=>'40%','NILAI'=>$data_visum[aspek_3]);
			...
			$tabel5{1} = array ('TOTAL'=>'Rata-Rata Nilai (Jumlah Bobot x Nilai)','NILAI'=>"<b>$data_visum[nilai_rata]</b>");
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Visum Bayangan                     <i>[hal $halaman dari $jumlah_halaman hal]</i>");
