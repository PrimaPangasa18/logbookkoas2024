<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_referat = mysqli_query($con,"SELECT * FROM `forensik_nilai_referat` WHERE `nim`='$nim_mhsw_forensik' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_referat);
		$halaman = 1;
		while ($data_referat = mysqli_fetch_array($daftar_referat))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI REFERAT KEPANITERAAN (STASE) KEDOKTERAN FORENSIK DAN MEDIKOLEGAL");
			...
			$tabel4{1} = array ('NO'=>'1','ASPEK'=>'Kerapian dan kelengkapan','BOBOT'=>'20%','NILAI'=>$data_referat[aspek_1]);
			$tabel4{2} = array ('NO'=>'2','ASPEK'=>'Kedalaman pembahasan','BOBOT'=>'30%','NILAI'=>$data_referat[aspek_2]);
			$tabel4{3} = array ('NO'=>'3','ASPEK'=>'Penguasaan materi dalam diskusi','BOBOT'=>'50%','NILAI'=>$data_referat[aspek_3]);
			...
			$tabel5{1} = array ('TOTAL'=>'Rata-Rata Nilai (Jumlah Bobot x Nilai)','NILAI'=>"<b>$data_referat[nilai_rata]</b>");
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Referat                                  <i>[hal $halaman dari $jumlah_halaman hal]</i>");
