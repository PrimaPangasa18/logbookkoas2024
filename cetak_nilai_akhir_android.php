...
$data_mhsw = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM `biodata_mhsw` WHERE `nim`='$nim_mhsw'"));
$data_stase = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM `$stase_id` WHERE `nim`='$nim_mhsw'"));
...
$delete_dummy_penyakit = mysqli_query($con, "DELETE FROM `jurnal_penyakit_dummy` WHERE `username`='$nim_mhsw'");
$delete_dummy_ketrampilan = mysqli_query($con, "DELETE FROM `jurnal_ketrampilan_dummy` WHERE `username`='$nim_mhsw'");
...
while ($data = mysqli_fetch_array($jurnal_penyakit)) {
	$insert_dummy = mysqli_query($con, "INSERT INTO `jurnal_penyakit_dummy`
		...
		VALUES
		('$data[id]', '$data[nim]','$data_mhsw[nama]','$data[angkatan]', '$data[grup]',
...
$grade_penyakit = ketuntasan_penyakit($con, $include_id, $target_id, $data_mhsw[nim], $nim_mhsw);

//Grade Jurnal Ketrampian
$grade_ketrampilan = ketuntasan_ketrampilan($con, $include_id, $target_id, $data_mhsw[nim], $nim_mhsw);
...
if ($id_stase != "M121") {
	...
	$pdf->ezTable(
		...
		"",
		array(
			'maxWidth' => 530, 'width' => 510, 'fontSize' => 8, 'showHeadings' => 0, 'shaded' => 0, 'showLines' => 1, 'titleFontSize' => 10, 'xPos' => 'center', 'xOrientation' => 'center', 'rowGap' => 1, 'showBgCol' => 0,
			'cols' => array('ITEM' => array('justification' => 'left'))
		)
	);
	$pdf->ezSetDy(-20, '');
	...
	$nip_kordik = mysqli_fetch_array(mysqli_query($con, "SELECT `username` FROM `admin` WHERE `stase`='$kode_kordik'"));
	...
	$kode_kordik1 = "K121";
	$kode_kordik2 = "K122";
	$nip_kordik1 = mysqli_fetch_array(mysqli_query($con, "SELECT `username` FROM `admin` WHERE `stase`='$kode_kordik1'"));
	$data_kordik1 = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM `dosen` WHERE `nip`='$nip_kordik1[username]'"));
	$nip_kordik2 = mysqli_fetch_array(mysqli_query($con, "SELECT `username` FROM `admin` WHERE `stase`='$kode_kordik2'"));
	$data_kordik2 = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM `dosen` WHERE `nip`='$nip_kordik2[username]'"));
...
$pdf->ezStream(array("Content-Disposition"=>"Nilai Akhir $id_stase $nim_mhsw.pdf"));
