...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_jurnal = mysqli_query($con, "SELECT * FROM `radiologi_nilai_jurnal` WHERE `nim`='$nim_mhsw_radiologi' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_jurnal);
		$halaman = 1;
		while ($data_jurnal = mysqli_fetch_array($daftar_jurnal)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI JOURNAL READING KEPANITERAAN (STASE) RADIOLOGI");
			...
			$tabel2[4] = array('item' => "Tanggal Presentasi/Ujian", 'isi' => ": " . "$tanggal_ujian");
			$tabel2[5] = array('item' => "Nama Jurnal", 'isi' => ": " . "$data_jurnal[nama_jurnal]");
			$tabel2[6] = array('item' => "Judul Artikel", 'isi' => ": " . "$data_jurnal[judul_paper]");
			$pdf->ezTable(
				$tabel2,
				$kolom2,
				"",
			...
			$tabel5[1] = array('TOTAL' => 'Nilai Rata-Rata:', 'NILAI' => $data_jurnal['nilai_rata']);
			$pdf->ezTable(
				$tabel5,
				$kolom5,
				"",
