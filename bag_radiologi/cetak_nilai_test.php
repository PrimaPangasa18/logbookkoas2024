...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_test = mysqli_query($con, "SELECT * FROM `radiologi_nilai_test` WHERE `nim`='$nim_mhsw_radiologi' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_test);
		$halaman = 1;
		while ($data_test = mysqli_fetch_array($daftar_test)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI TEST KEPANITERAAN (STASE) RADIOLOGI");
			...
			$tabel6[3] = array('ITEM' => 'Kordik Kepaniteraan (Stase) Radiologi');
			$tabel6[4] = array('ITEM' => '');
