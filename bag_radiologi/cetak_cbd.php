<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_cbd = mysqli_query($con,"SELECT * FROM `radiologi_nilai_cbd` WHERE `nim`='$nim_mhsw_radiologi' AND `kasus`='$kasus' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_cbd = mysqli_fetch_array($daftar_cbd))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI CBD - $kasus_up KEPANITERAAN (STASE) RADIOLOGI");
			...
			$tabel2{4} = array ('item'=>"Jenis Kasus", 'isi'=>": "."$data_cbd[kasus]");
			...
			$tabel4{1} = array ('NO'=>'1','ASPEK'=>"Kemampuan membuat permintaan dan mengintepretasi pemeriksaan radiologi",'NILAI'=>$data_cbd[aspek_1]);
			$tabel4{2} = array ('NO'=>'2','ASPEK'=>"Kemampuan Penulisan",'NILAI'=>$data_cbd[aspek_2]);
			$tabel4{3} = array ('NO'=>'3','ASPEK'=>"Penyajian",'NILAI'=>$data_cbd[aspek_3]);
			$tabel4{4} = array ('NO'=>'4','ASPEK'=>"Penguasaan Materi",'NILAI'=>$data_cbd[aspek_4]);
			$tabel4{5} = array ('NO'=>'5','ASPEK'=>"Tanya Jawab",'NILAI'=>$data_cbd[aspek_5]);
			$tabel4{6} = array ('NO'=>'6','ASPEK'=>"Keaktifan",'NILAI'=>$data_cbd[aspek_6]);
			...
			$tabel5{1} = array ('TOTAL'=>'Nilai Rata-Rata:','NILAI'=>"<b>$data_cbd[nilai_rata]</b>");
			...
			$pdf->ezSetDy(-10,'');
			$pdf->addText(50,25,10,"$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Kasus CBD - $kasus   <i>[hal $halaman dari $jumlah_halaman hal]</i>");
