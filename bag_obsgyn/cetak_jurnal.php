<?php
	...
	{
		if ($_COOKIE[level]=='5') $nim_mhsw_obsgyn = $_COOKIE[user];
		if ($_COOKIE[level]=='1' OR $_COOKIE[level]=='2' OR $_COOKIE[level]=='3') $nim_mhsw_obsgyn = $_GET[nim];

		$data_mhsw = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `biodata_mhsw` WHERE `nim`='$nim_mhsw_obsgyn'"));
		$datastase_mhsw = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `stase_M111` WHERE `nim`='$nim_mhsw_obsgyn'"));
		...
		$daftar_jurnal = mysqli_query($con,"SELECT * FROM `obsgyn_nilai_jurnal` WHERE `nim`='$nim_mhsw_obsgyn' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_jurnal = mysqli_fetch_array($daftar_jurnal))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI JOURNAL READING");
			$tabel1{2} = array ('item'=>"KEPANITERAAN (STASE) ILMU KEBIDANAN DAN PENYAKIT KANDUNGAN");
			...
			$tabel2{6} = array ('item'=>"Nama Jurnal", 'isi'=>": "."$data_jurnal[nama_jurnal]");
			$tabel2{7} = array ('item'=>"Judul Artikel", 'isi'=>": "."$data_jurnal[judul_paper]");
			...
			$tabel3{1} = array ('NO'=>'<b>No</b>','ASPEK'=>'<b>Aspek Yang Dinilai</b>','NILAI'=>'<b>Nilai</b>');
			...
			//Nilai Aspek 1 - 5
			$aspek1="Transparansi";
			$aspek2="Cara Penyajian";
			$aspek3="Penguasaan Materi";
			$aspek4="Keaktifan";

			$kolom4 = array ('NO'=>'','ASPEK'=>'','NILAI'=>'');
			$tabel4{1} = array ('NO'=>'1','ASPEK'=>$aspek1,'NILAI'=>$data_jurnal[aspek_1]);
			$tabel4{2} = array ('NO'=>'2','ASPEK'=>$aspek2,'NILAI'=>$data_jurnal[aspek_2]);
			$tabel4{3} = array ('NO'=>'3','ASPEK'=>$aspek3,'NILAI'=>$data_jurnal[aspek_3]);
			$tabel4{4} = array ('NO'=>'4','ASPEK'=>$aspek4,'NILAI'=>$data_jurnal[aspek_4]);
			...
			$tabel5{1} = array ('TOTAL'=>'Rata-Rata Nilai (Total Nilai / 4):','NILAI'=>$data_jurnal[nilai_rata]);
			...
			//Catatan
			$kolom5a = array ('ITEM'=>'');
			$tabel5a{1} = array ('ITEM'=>"<b>Catatan Dosen Penilai:</b>\r\n\r\n<i>$data_jurnal[catatan]</i>");
