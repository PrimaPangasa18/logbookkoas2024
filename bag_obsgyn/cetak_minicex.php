<?php
	...
	if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND ($_COOKIE['level']==1 OR $_COOKIE['level']==2 OR $_COOKIE['level']==3 OR $_COOKIE['level']==5))
	{
		...
		$daftar_minicex = mysqli_query($con,"SELECT * FROM `obsgyn_nilai_minicex` WHERE `nim`='$nim_mhsw_obsgyn' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		...
		while ($data_minicex = mysqli_fetch_array($daftar_minicex))
		{
			//Judul
			$kolom1 = array ('item'=>"");
			$tabel1{1} = array ('item'=>"NILAI UJIAN MINI-CEX (MINI CLINICAL EXAMINATION)");
			...
			$tabel2{6} = array ('item'=>"Diagnosis", 'isi'=>": "."$data_minicex[diagnosis]");
