...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_cbd = mysqli_query($con, "SELECT * FROM `obsgyn_nilai_cbd` WHERE `nim`='$nim_mhsw_obsgyn' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_cbd);
		$halaman = 1;
		while ($data_cbd = mysqli_fetch_array($daftar_cbd)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI CASE-BASED DISCUSSION (CBD)");
			...
			//Nilai Aspek 1 - 5
			if ($data_cbd['observasi_1'] == "0") $obs1 = "TIDAK";
			else $obs1 = "YA";
			if ($data_cbd['observasi_2'] == "0") $obs2 = "TIDAK";
			else $obs2 = "YA";
			if ($data_cbd['observasi_3'] == "0") $obs3 = "TIDAK";
			else $obs3 = "YA";
			if ($data_cbd['observasi_4'] == "0") $obs4 = "TIDAK";
			else $obs4 = "YA";
			if ($data_cbd['observasi_5'] == "0") $obs5 = "TIDAK";
			else $obs5 = "YA";
			if ($data_cbd['observasi_6'] == "0") $obs6 = "TIDAK";
			else $obs6 = "YA";
			if ($data_cbd['observasi_7'] == "0") $obs7 = "TIDAK";
			else $obs7 = "YA";
