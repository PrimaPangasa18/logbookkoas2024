...
if (!empty($_COOKIE['user']) AND !empty($_COOKIE['pass']) AND $_COOKIE[level]==2)
{
  ...
  if ($_POST['cari']=="CARI")
  {
    ...
    if ($jml>=1)
    {
      echo "<font style=\"font-size:0.625em\"><i>(Klik username untuk akses kegiatan dosen / residen)</i></font><p>";
      echo "<table style=\"border:0;box-shadow: 10px 10px 20px rgba(0,0,0,0.4)\">";
      echo "<tr class=\"ganjil\">";
        echo "<td style=\"width:50px;text-align:center\">No</td>";
        echo "<td style=\"width:250px;text-align:center\">Username (NIP/NIK/Kode)</td>";
      ...
      while ($data=mysqli_fetch_array($user_kunci))
      {
        ...
        echo "<tr class=\"genap\">";
          ...
          echo "<td><a href=\"filter_kegiatan_dosen_admin.php?user_name=$data_dosen[nip]\">$data_dosen[nip]</a></td>";
