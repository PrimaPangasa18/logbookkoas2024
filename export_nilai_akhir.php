...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3)) {
		...
		$kepaniteraan = strtoupper($stase[kepaniteraan]);
		$cap = "<table align=center border=1px><th colspan='11' style=\"text-align:center; font-size:14px;\"><b>REKAP NILAI BAGIAN / KEPANITERAAN (STASE) $kepaniteraan</b></th>";
		$title = "<tr><b>
					<th>No</th>
					<th>Nama</th>
					<th>NIM</th>
					...
					<th>Jurnal Penyakit (%)</th>
					<th>Jurnal Ketrampilan (%)</th>
					<th>Penilaian Bagian</th>
					<th>Nilai Akhir</th>
					<th>Grade Akhir</th>
					</b></tr>"; //TABLE HEADER
		...
		while ($data_mhsw = mysqli_fetch_array($data_mhsw_all)) {
			...
			$body .= "<tr>
						...
						<td>" . $tgl_mulai . "</td>
						<td>" . $tgl_selesai . "</td>
						<td style=\"text-align:center;\">" . $grade_penyakit . "</td>
						<td style=\"text-align:center;\">" . $grade_ketrampilan . "</td>
						<td style=\"text-align:center;\">" . $nilai_bagian . "</td>
						<td style=\"text-align:center;\">" . $nilai_akhir . "</td>
						<td style=\"text-align:center;\">" . $grade_akhir . "</td>
						</tr>";
		...
		$exp->exportWithPage($rsql, "Rekap Nilai Akhir Kepaniteraan $stase[kepaniteraan] $tgl");
	} else
		echo "
	<script>
		window.location.href=\"accessdenied.php\";
