...
$data_koas = mysqli_query($con,"SELECT * FROM `import_jadwal_ppp70_mulai_04okt2021`");
$no = 0;
while ($data=mysqli_fetch_array($data_koas))
{
  ...
  while ($datastase=mysqli_fetch_array($data_stase))
  {
    ...
    $delete_jadwal = mysqli_query($con,"DELETE FROM `$stase_id` WHERE `nim`='$data[nim]' AND `tgl_mulai`>'2021-10-03'");
