...
<?php
	...
	{
		include "menu2.php";
		echo "<div class=\"text_header\">EDIT KETRAMPILAN</div>";
		echo "<br><br><br><fieldset class=\"fieldset_art\">
	    	<legend align=left><font style=\"color:black;font-style:italic;font-size:0.825em;\">[user: $_COOKIE[nama], $_COOKIE[gelar]]</font></legend>";
		echo "<center><h4><font style=\"color:#006400;text-shadow:1px 1px black;\">EDIT KETRAMPILAN</font></h4>";

		echo "<form method=\"POST\" action=\"$_SERVER[PHP_SELF]\">";
		echo "<input type=\"hidden\" name=\"id\" value=\"$_GET[id]\">";
		$data_ketrampilan = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM `daftar_ketrampilan` WHERE `id`='$_GET[id]'"));

		echo "<table style=\"width:75%\">";
		//Nama Ketrampilan
	  echo "<tr class=\"ganjil\">";
	  echo "<td class=\"td_mid\" style=\"width:30%\">Nama Ketrampilan</td>";
	  echo "<td class=\"td_mid\" style=\"width:70%\">";
	  echo "<textarea name=\"ketrampilan\" rows=\"1\" class=\"select_art\" style=\"border:0.5px solid grey;border-radius:5px;\">$data_ketrampilan[ketrampilan]</textarea>";
	  echo "</td>";
	  echo "</tr>";
		//Level Ketrampilan SKDI
		echo "<tr class=\"ganjil\">";
		echo "<td>Level Ketrampilan (SKDI) </td>";
		echo "<td>";
		echo "<select class=\"select_art\" name=\"skdi_level\" id=\"skdi_level\">";
		if ($data_ketrampilan[skdi_level]=='-') echo "<option value=\"-\"> - </option>";
		else echo "<option value=\"$data_ketrampilan[skdi_level]\">Level $data_ketrampilan[skdi_level]</option>";
		...
		echo "<tr class=\"ganjil\">";
		echo "<td>Level Ketrampilan (Kepmenkes) </td>";
		echo "<td>";
		echo "<select class=\"select_art\" name=\"k_level\" id=\"k_level\">";
		if ($data_ketrampilan[k_level]=='-') echo "<option value=\"-\"> - </option>";
		else echo "<option value=\"$data_ketrampilan[k_level]\">Level $data_ketrampilan[k_level]</option>";
		echo "<option value=\"-\"> - </option>";
		echo "<option value=\"1\">Level 1</option>";
		echo "<option value=\"2\">Level 2</option>";
		echo "<option value=\"3\">Level 3</option>";
		...
		echo "<tr class=\"ganjil\">";
		echo "<td>Level Ketrampilan (IPSG) </td>";
		echo "<td>";
		...
		echo "<tr class=\"ganjil\">";
		echo "<td>Level Ketrampilan (Muatan Lokal) </td>";
		echo "<td>";
		echo "<select class=\"select_art\" name=\"kml_level\" id=\"kml_level\">";
		if ($data_ketrampilan[kml_level]=='-') echo "<option value=\"-\"> - </option>";
		else echo "<option value=\"$data_ketrampilan[kml_level]\">Level $data_ketrampilan[kml_level]</option>";
		echo "<option value=\"-\"> - </option>";
		echo "<option value=\"1\">Level 1</option>";
		echo "<option value=\"2\">Level 2</option>";
		echo "<option value=\"3\">Level 3</option>";
		...
		echo "<td>";
			...
			while ($data=mysqli_fetch_array($daftar_stase))
			{
				...
				if ($data_ketrampilan[$nama_cek]=='1')
				{
					...
					echo "<input type=\"text\" style=\"width:100%;text-align:center\" name=\"$target\" id=\"$target\" value=\"$data_ketrampilan[$target]\"></td>";
				}
				else
				{
		...
		if ($_POST[batal]=="BATAL")
		{
			echo "
			<script>
				window.location.href=\"cari_ketrampilan.php\";
			</script>
			";
		}
		...
		{
			$delete_ketrampilan = mysqli_query($con,"DELETE FROM `daftar_ketrampilan` WHERE `id`='$_POST[id]'");
			echo "
			<script>
				window.location.href=\"cari_ketrampilan.php?status=HAPUS\";
			</script>
			";
		}
		...
		if ($_POST[ubah]=="UBAH")
		{
			...
			$update_ketrampilan = mysqli_query($con,"UPDATE `daftar_ketrampilan`
				SET
				`ketrampilan`='$_POST[ketrampilan]',`skdi_level`='$_POST[skdi_level]',`k_level`='$_POST[k_level]',
			...
			echo "
			<script>
				window.location.href=\"cari_ketrampilan.php?status=UBAH&id=\"+\"$_POST[id]\";
			</script>
			";
