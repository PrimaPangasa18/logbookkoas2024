...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_presentasi = mysqli_query($con, "SELECT * FROM `thtkl_nilai_presentasi` WHERE `nim`='$nim_mhsw_thtkl' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_presentasi);
		$halaman = 1;
		while ($data_presentasi = mysqli_fetch_array($daftar_presentasi)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI PRESENTASI KASUS KEPANITERAAN (STASE) ILMU KESEHATAN THT-KL");
			...
			$tabel4[1] = array('NO' => '1', 'ASPEK' => 'Diskusi tentang Anamnesis', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_1']);
			$tabel4[2] = array('NO' => '2', 'ASPEK' => 'Diskusi tentang Pemeriksaan Fisik', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_2']);
			$tabel4[3] = array('NO' => '3', 'ASPEK' => 'Diskusi tentang Pemeriksaan Penunjang', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_3']);
			$tabel4[4] = array('NO' => '4', 'ASPEK' => 'Diskusi tentang Diagnosis dan Diagnosis Banding', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_4']);
			$tabel4[5] = array('NO' => '5', 'ASPEK' => 'Diskusi tentang Penatalaksanaan (farmakoterapi atau/dan non farmakoterapi)', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_5']);
			$tabel4[6] = array('NO' => '6', 'ASPEK' => 'Diskusi tentang Komunikasi, Informasi dan Edukasi kepada Pasien atau/dan keluarga', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_6']);
			$tabel4[7] = array('NO' => '7', 'ASPEK' => 'Diskusi tentang Profesionalitas', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_7']);
			$tabel4[8] = array('NO' => '8', 'ASPEK' => 'Presentasi (PPT, kejelasan penyampaian presentasi, pemahaman kasus)', 'BOBOT' => '20%', 'NILAI' => $data_presentasi['aspek_8']);
			$tabel4[9] = array('NO' => '9', 'ASPEK' => 'Pembuatan Makalah', 'BOBOT' => '10%', 'NILAI' => $data_presentasi['aspek_9']);
			$pdf->ezTable(
				$tabel4,
				$kolom4,
				"",
			...
			$tabel5[1] = array('TOTAL' => 'Rata-Rata Nilai (Jumlah Bobot x Nilai):', 'NILAI' => "<b>$data_presentasi[nilai_rata]</b>");
			$pdf->ezTable(
				$tabel5,
				$kolom5,
				"",
			...
			$pdf->ezSetDy(-10, '');
			$pdf->addText(50, 25, 10, "$data_mhsw[nama] ($data_mhsw[nim]) - Cetak Nilai Presentasi Kasus                     <i>[hal $halaman dari $jumlah_halaman hal]</i>");
