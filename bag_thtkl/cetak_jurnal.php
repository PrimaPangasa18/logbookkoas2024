...
if (empty($_COOKIE['user']) || empty($_COOKIE['pass'])) {
	...
	if (!empty($_COOKIE['user']) and !empty($_COOKIE['pass']) and ($_COOKIE['level'] == 1 or $_COOKIE['level'] == 2 or $_COOKIE['level'] == 3 or $_COOKIE['level'] == 5)) {
		...
		$daftar_jurnal = mysqli_query($con, "SELECT * FROM `thtkl_nilai_jurnal` WHERE `nim`='$nim_mhsw_thtkl' AND `status_approval`='1' ORDER BY `tgl_approval` ASC");
		$jumlah_halaman = mysqli_num_rows($daftar_jurnal);
		$halaman = 1;
		while ($data_jurnal = mysqli_fetch_array($daftar_jurnal)) {
			//Judul
			$kolom1 = array('item' => "");
			$tabel1[1] = array('item' => "NILAI JOURNAL READING KEPANITERAAN (STASE) ILMU KESEHATAN THT-KL");
			...
			$tabel2[4] = array('item' => "Journal Reading Ke-", 'isi' => ": " . "$data_jurnal[jurnal_ke]");
			...
			$tabel4[1] = array('NO' => '1', 'ASPEK' => 'Menerjemahkan Journal Reading', 'BOBOT' => '30%', 'NILAI' => $data_jurnal['aspek_1']);
			$tabel4[2] = array('NO' => '2', 'ASPEK' => 'Relevansi Pembicaraan', 'BOBOT' => '50%', 'NILAI' => $data_jurnal['aspek_2']);
			$tabel4[3] = array('NO' => '3', 'ASPEK' => 'Keterampilan Berkomunikasi', 'BOBOT' => '20%', 'NILAI' => $data_jurnal['aspek_3']);
			$pdf->ezTable(
				$tabel4,
				$kolom4,
				"",
